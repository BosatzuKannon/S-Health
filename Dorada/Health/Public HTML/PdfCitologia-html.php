<?php
	...
	$strConsulta = "SELECT * from hlt_atencion_citologia where caso =  '$caso'";
	...
	if (file_exists('datosxml/CitologiaXml.xml')) {
    $xml = simplexml_load_file('datosxml/CitologiaXml.xml');
	...
	<fieldset>
		...
		<label>Atencion: Citologia</label><br>	
	...
	<fieldset>
		...
		<label>'.$xml->DRP_ENF_CRONICAS->es.': '.$xml->DRP_ENF_CRONICAS->es->option[intval($psic['drp_enf_cronicas'])].'</label>	<br>
		<label>'.$xml->DRP_ENF_INFECTO->es.': '.$xml->DRP_ENF_INFECTO->es->option[intval($psic['drp_enf_infecto'])].'</label>	<br>
		<label>'.$xml->DRP_ALERGIAS->es.': '.$xml->DRP_ALERGIAS->es->option[intval($psic['drp_alergias'])].'</label>	<br>
		<label>'.$xml->DRP_INTOXICACIONES->es.': '.$xml->DRP_INTOXICACIONES->es->option[intval($psic['drp_intoxicaciones'])].'</label><br>
		<label>'.$xml->DRP_ENF_TRAUMAS->es.': '.$xml->DRP_ENF_TRAUMAS->es->option[intval($psic['drp_enf_traumas'])].'</label><br>
		<label>'.$xml->DRP_MEDICAMENTOS->es.': '.$xml->DRP_MEDICAMENTOS->es->option[intval($psic['drp_medicamentos'])].'</label><br>
		<label>'.$xml->DRP_CIRUGIA_HOSPI->es.': '.$xml->DRP_CIRUGIA_HOSPI->es->option[intval($psic['drp_cirugia_hospi'])].'</label><br>
	</fieldset>
	<fieldset>
	<legend>IV. Antecedentes Familiares</legend>
		<label>'.$xml->DRP_ENF_METABOLICAS->es.': '.$xml->DRP_ENF_METABOLICAS->es->option[intval($psic['drp_enf_metabolicas'])].'</label><br>
		<label>'.$xml->DRP_ENF_CRONICAS1->es.': '.$xml->DRP_ENF_CRONICAS1->es->option[intval($psic['drp_enf_cronicas1'])].'</label><br>
		<label>'.$xml->DRP_ENF_INFECTO1->es.': '.$xml->DRP_ENF_INFECTO1->es->option[intval($psic['drp_enf_infecto1'])].'</label><br>
		<label>'.$xml->DRP_CANCER->es.': '.$xml->DRP_CANCER->es->option[intval($psic['drp_cancer'])].'</label><br>
		<label>'.$xml->DRP_ENF_CARDIOVASCULARES->es.': '.$xml->DRP_ENF_CARDIOVASCULARES->es->option[intval($psic['drp_enf_cardiovasculares'])].'</label><br>
		<label>'.$xml->DRP_HIPERLIPIDEMIAS->es.': '.$xml->DRP_HIPERLIPIDEMIAS->es->option[intval($psic['drp_hiperlipidemias'])].'</label><br>
		<label>'.$xml->DRP_MENTALES->es.': '.$xml->DRP_MENTALES->es->option[intval($psic['drp_mentales'])].'</label><br>
		<label>'.$xml->TXT_MENARQUIA->es.': '.$psic['txt_menarquia'].'</label><br>
		<label>'.$xml->TXT_SEXARCA->es.': '.$psic['txt_sexarca'].'</label><br>
		<label>'.$xml->TXT_ITS->es.': '.$psic['txt_its'].'</label><br>
		<label>'.$xml->TXT_PAREJAS_SEXUALES->es.': '.$psic['txt_parejas_sexuales'].'</label><br>
		<label>'.$xml->TXT_FUM->es.': '.$psic['txt_fum'].'</label><br>
		<label>'.$xml->DATE_ULT_CITOLOGIA->es.': '.$psic['date_ult_citologia'].'</label><br>
		...
		<label>'.$xml->TXT_V->es.': '.$psic['txt_v'].'</label><br>
		<label>'.$xml->TXT_M->es.': '.$psic['txt_m'].'</label><br>
		<label>'.$xml->TXT_FUP->es.': '.$psic['txt_fup'].'</label><br>
		<label>'.$xml->TXT_METODO_PLANIFICACION->es.': '.$psic['txt_metodo_planificacion'].'</label><br>
		<label>'.$xml->TXT_ESPECULOSCOPIA->es.': '.$psic['txt_especuloscopia'].'</label><br>
		<label>'.$xml->AREA_PLAN->es.': '.$psic['area_plan'].'</label><br>
		<label>'.$xml->AREA_CONDUCTA->es.': '.$psic['area_conducta'].'</label><br>
	</fieldset>
	</body>
	</html>';
